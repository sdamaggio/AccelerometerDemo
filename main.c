    ...
    switch(p_event->type)
    {
        case NRF_DRV_TWI_EVT_DONE:
            if ((p_event->type == NRF_DRV_TWI_EVT_DONE) && (p_event->xfer_desc.type == NRF_DRV_TWI_XFER_TX))
            {
                ...
                err_code = nrf_drv_twi_rx(&m_twi_mma_7660, MMA8451_ADDR, &res, sizeof(res));
                APP_ERROR_CHECK(err_code);
                NRF_LOG_PRINTF("res= %d p_event->type= %d\r\n", res, p_event->type);
            }
            else
            {
                //read_data(&m_sample);
            	NRF_LOG_PRINTF("READ DATA res= %d\r\n", res);
                m_xfer_done = true;
            }
            break;

        default:
        	NRF_LOG_PRINTF("RX case default: p_event->type= %d\r\n", p_event->type);
            break;
    }
}
...
void twi_init (void)
{
    ...
    const nrf_drv_twi_config_t twi_mma_7660_config = {
       .scl                = ARDUINO_I2C_SCL_PIN,
       .sda                = ARDUINO_I2C_SDA_PIN,
...
int main(void)
{
   ...
   // int a = __GNUC__, c = __GNUC_PATCHLEVEL__;
    ...
    err_code = nrf_drv_twi_tx(&m_twi_mma_7660, MMA8451_ADDR, &reg, sizeof(reg), true);
    APP_ERROR_CHECK(err_code);
